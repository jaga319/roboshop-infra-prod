...
resource "aws_cloudfront_distribution" "infome" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.infome.domain_name
        zone_id = aws_cloudfront_distribution.infome.hosted_zone_id
      }
    }
  ]
